...
const APIKey = '4c8432fe1cd274d9d8c8e0be54f6fa3b';
...
if(navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(position => {
        ...
        alert(`Vous avez refusé la géolocalisation, l'application ne peus pas fonctionner, veuillez l'activer!`)
    });
}
...
function AppelAPI(long, lat) {
    ...
    exclude=minutely&units=metric&lang=fr&appid=${APIKey}`)
    .then( (reponse) => {
        return reponse.json() ;
    })
    .then((data) => {
       // console.log(data);
        ...
        for (let k = 0; k < tabJoursEndOrdre.length; k++) {
             jourDiv[k].innerText  = tabJoursEndOrdre[k].slice(0,3);
